...
def generate_planar_graph(
    ...
    Args:
        ...
        method: Generation method ("delaunay" or "boltzmann")
        epsilon: Size tolerance for Boltzmann sampler (0=exact, >0=approximate)
        require_connected: Whether to require connectivity (Boltzmann only)
        with_embedding: Whether to return planar embedding (Boltzmann only)
        allow_multiproc: Whether to allow parallel processing (Boltzmann only)
    ...
    Boltzmann advantages:
        - Uniform random sampling from all planar graphs
        - Better size control for large graphs
        - Exact connectedness guarantee
        - Theoretical soundness

    Delaunay advantages:
        - Faster for small graphs (< 100 nodes)
...
def _generate_planar_graph_boltzmann(
    ...
    if weighted:
        G = add_edge_weights(G, seed + 1000)
...
def save_results(
    ...
    Args:
        ...
        resistance_matrix: Numpy resistance matrix
        resistance_multiset: List of resistance distances
        graph_metadata: Dictionary containing graph metadata
        output_dir: Base output directory (unused, kept for compatibility)
        graph_id: Optional graph identifier
    """
    adj_hash = hashlib.sha256(adjacency_matrix.tobytes()).hexdigest()[:8]

    n_verts = graph_metadata["n_nodes"]
    m_edges = graph_metadata["m_edges"]
    method = graph_metadata["generation_method"]

    dir_name = f"{n_verts}verts_{m_edges}edges_{adj_hash}"
    output_path = Path("./graphs/planar") / method / dir_name
    output_path.mkdir(parents=True, exist_ok=True)

    adj_matrix_path = output_path / "adjacency_matrix.json"
    with open(adj_matrix_path, "w") as f:
        json.dump(adjacency_matrix.tolist(), f)

    res_matrix_path = output_path / "resistance_matrix.json"
    with open(res_matrix_path, "w") as f:
        json.dump(resistance_matrix.tolist(), f)

    res_multiset_path = output_path / "resistance_multiset.json"
    with open(res_multiset_path, "w") as f:
        json.dump(resistance_multiset, f)

    print(f"Saved results to {output_path}")
    print(f"  adjacency_matrix.json")
    print(f"  resistance_matrix.json")
    print(f"  resistance_multiset.json")
...
def load_results(filepath: Path) -> dict:
    ...
    Args:
        filepath: Path to the directory containing JSON files

    Returns:
        Dictionary containing adjacency matrix, resistance matrix, resistance multiset, and metadata
    """
    adj_matrix_path = filepath / "adjacency_matrix.json"
    res_matrix_path = filepath / "resistance_matrix.json"
    res_multiset_path = filepath / "resistance_multiset.json"

    with open(adj_matrix_path, "r") as f:
        adjacency_matrix = np.array(json.load(f))

    with open(res_matrix_path, "r") as f:
        resistance_matrix = np.array(json.load(f))

    with open(res_multiset_path, "r") as f:
